...
/*   Updated: 2023/10/31 09:30:41 by flverge          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	preci_0_width_1_x(unsigned long nb, int len_nb, t_flags flags, int hashtag)
{
	...
	else
	{
		if (!flags.zero)
		{
			real_width = flags.width - determine_hashtag(flags) - len_nb;
			if (real_width > 0)
			{
				if (flags.zero)
...
{
	if (!flags.width)
		flags.lenght_print = preci_1_width_0(nb, len_nb, flags, hashtag);
	else
...
int	preci_1_width_1_x(unsigned long nb, int len_nb, t_flags flags)
{
	...
	if (real_precision < 0)
		real_precision = 0;
	real_width = flags.width - determine_hashtag(flags) - len_nb - real_precision;
	if (real_width < 0)
		real_width = 0;
	if (!flags.precision && !nb)
		return (zero_case_x(flags));
	if (!flags.minus_sign)
	{
		print_width_space(real_width);
		if (nb < 0)
